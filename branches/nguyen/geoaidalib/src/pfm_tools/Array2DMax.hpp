...
template < class Value >
  Array2DMax < Value >::Array2DMax(const Array2D < Value > &src,
...
template < class Value >
  bool Array2DMax < Value >::getSrcVal(int x, int y, Value & val,
                                       const Array2D < Value > &src)
{
  if ((x < 0) 
      || (y < 0) || (x >= Array2D<Value>::xsize()) 
      || (y >= Array2D<Value>::ysize()))
    return false;
  val = src.get(x, y);
...
template < class Value >
  void Array2DMax < Value >::CalcPoint(int xd, int yd,
                                       const Array2D < Value > &src,
                                       int fsizex, int fsizey)
{
  Value sumval;
