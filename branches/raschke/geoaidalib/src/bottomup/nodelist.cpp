        ...
        if(!newReg) selected->insert(node->key(),node);			
          ...
          for (int y=se->ury; y<=se->lly; y++)
            for (int x=se->llx; x<=se->urx; x++) {
              if (outImage_.getPixel(x,y)==oldID) {
                if(oldID != freeID) {
                  int newID=freeID;
...
//    node->id(),
//    bb.getPixel(node->id(),0),
//    bb.getPixel(node->id(),1),
//    bb.getPixel(node->id(),2),
//    bb.getPixel(node->id(),3));
#ifdef WIN32
...
//  int getPixel(int x, int y, int channel=0);
  /** get a pixel */
//  int getPixel(const void *ptr);
//Quelltyp konnte von keinem Konstruktor angenommen werden, oder die Ueberladungsaufloesung des Konstruktors ist mehrdeutig
    node->replace("llx",&QString((float)(bb.getPixel(node->id(),0))));
    node->replace("lly",&QString((float)(bb.getPixel(node->id(),1))));
    node->replace("urx",&QString((float)(bb.getPixel(node->id(),2))));
    node->replace("ury",&QString((float)(bb.getPixel(node->id(),3))));
#else
    node->replace("llx",bb.getPixel(node->id(),0));
    node->replace("lly",bb.getPixel(node->id(),1));
    node->replace("urx",bb.getPixel(node->id(),2));
    node->replace("ury",bb.getPixel(node->id(),3));
#endif
    // XXX Was ist mit voellig ueberdeckten regionen ??
    if( bb.getPixel(node->id(),0)==0 && bb.getPixel(node->id(),1)==0 &&
        bb.getPixel(node->id(),2)==0 && bb.getPixel(node->id(),3)==0 )
