...
namespace {
  ...
  Image normalizeImage(Image &source) {
    ...
    Image result(typeid(float), sizeX, sizeY);
	  ...
	  float min = source.matrixMin();
	  float range = source.matrixMax() - min;
  ...
  Image convolve(const Image &source, const int (&coefficients)[9],
  ...
  bool trace(Image &result, const Image &intensities, const Image &angles,
